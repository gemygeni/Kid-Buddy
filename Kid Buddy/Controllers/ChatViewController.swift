...
class ChatViewController: UIViewController, UIGestureRecognizerDelegate {
    ...
    func downloadMessages(completionHandler: @escaping (_: [Message], _: [String]) -> Void) {
    ...
    func beginBatchFetch(completionHandler: @escaping () -> Void ) {
        ...
        downloadMessages {[weak self] newMessages, newMessagesIds in
            ...
            self?.endReached = newMessages.isEmpty
    ...
    @objc func image(_ image: UIImage, didFinishSavingWithError error: Error?, contextInfo: UnsafeRawPointer) {
        if let error = error {
            ...
            let errorAlertController = UIAlertController(title: "Saving error", message: error.localizedDescription, preferredStyle: .alert)
        errorAlertController.addAction(UIAlertAction(title: "OK", style: .default))
            present(errorAlertController, animated: true)
        } else {
            let savingAlertController = UIAlertController(title: "Saved!", message: "Your image has been saved to your photos.", preferredStyle: .alert)
        savingAlertController.addAction(UIAlertAction(title: "OK", style: .default))
...
extension ChatViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            self.messages = []
            self.messagesIds = []
            selectedReference.removeValue { [weak self] error, _ in
                if error != nil {
