...
class SignUpViewController: UIViewController {
    ...
    private func handleSignUp() {
        ...
        guard let name = nameTextField.text, let email = emailTextField.text, let password = passwordTextField.text, !password.isEmpty, !email.isEmpty, !password.isEmpty else {return}
        firebaseAuth.createUser(withEmail: email, password: password) { result, error in
            ...
            self.usersReference.child(UId).updateChildValues(userInfo) { error, _ in
                if let error = error {
